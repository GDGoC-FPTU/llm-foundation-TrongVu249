...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = "| Prompt | Model | Response | Latency | Tokens (In/Out) | Cost (USD) |"
    separator = "|--------|-------|----------|---------|-----------------|------------|"
    ...
    for result in results:
        ...
        for model_key, display_name in model_display.items():
            if model_key not in result:
                continue
            data = result[model_key]
            response = data["response"][:50]
            latency = f"{data['latency']:.2f}s"
            tokens = f"{data['input_tokens']}/{data['output_tokens']}"
            cost = f"${data['cost']:.8f}"
            rows.append(f"| {prompt} | {display_name} | {response} | {latency} | {tokens} | {cost} |")
